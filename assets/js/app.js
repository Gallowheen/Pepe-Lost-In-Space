$(document).ready(function(){
	...
	function draw(){
		...
		//anticheat by https://github.com/zswang/jdetects
		//Thanks ;)
		if(tick >= tickSpeed){
		...
		if(!demoIsOn){
			...
			// 	//Si c'est un autre navigateur que chrome
			// 	if(isFirefox || isEdge || isSafari || isOpera || isIE){
			// 		//Si la console est présente
			// 		window.onresize = function(){
			// 			if (screen.height - $(window).height() >= 300){

			// 			}
			// 			else{
			// 				element = document.querySelector('#devtool-status');
			// 				element.innerHTML = "";
			// 				showWarning = false;
			// 			}
			// 		}
			// 		if (!demoIsOn && screen.height - $(window).height() >= 300){
			// 	 		score = 0;

			// 	 		if(!showWarning){
			// 	 			element = document.querySelector('#devtool-status');
			// 				element.innerHTML = "Cheating is for loosers, please close your console !";
			// 				showWarning = true;	
			// 	 		}
			// 	 		life -= 1;
			...
			// 	}
			// 	else{
			// 		if (!demoIsOn && status == "on"){
			// 	 		score = 0;
			// 	 		lastShield = numberOfShield;
			// 	 		lastLife = life;
